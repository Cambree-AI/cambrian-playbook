// api-aws/shared/fetch-ssrf.js — SSRF validation for the Lambda /api/fetch
// (issue #87): verbatim copy of api/_fetch-ssrf.js below this header.
//
// KEEP IN SYNC with api/_fetch-ssrf.js until the Vercel copy is
// decommissioned — the full SSRF suite (tests/fetch-ssrf/ssrf.test.js) runs
// against BOTH copies (`--module=api-aws` selects this one), so any drift
// fails CI. All validation is pure functions with injectable DNS — no env,
