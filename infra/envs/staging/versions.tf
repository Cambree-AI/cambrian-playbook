# staging environment layer — account 865526619955 (cambree-staging).
# Applied by CI (GitHub Actions OIDC, .github/workflows/terraform.yml) on
# merge to the `staging` branch; bootstrapped once locally via
# OrganizationAccountAccessRole (see infra/github-oidc/README.md).

terraform {
  required_version = ">= 1.10.0"

  required_providers {
    aws = {
      source  = "hashicorp/aws"
      version = "~> 5.0"
    }
  }

  # Bucket is created by state.tf in this same layer. Bootstrap done
  # 2026-08-20: first apply ran with local state, then state was migrated
  # here via `terraform init -migrate-state`.
  backend "s3" {
    bucket       = "cambree-staging-terraform-state-865526619955"
    key          = "env/terraform.tfstate"
    region       = "us-east-2"
    use_lockfile = true
  }
}

provider "aws" {
  region = "us-east-2"

  # Hard guard: refuse to plan/apply against any account but cambree-staging,
  # whatever credentials this runs under.
  allowed_account_ids = ["865526619955"]

  default_tags {
    tags = {
      Env       = "staging"
      ManagedBy = "terraform"
      Repo      = "Cambree-AI/cambrian-playbook"
    }
  }
}
